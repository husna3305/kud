...
class Ion_auth
{
	...
	public function is_admin($id = FALSE)
	{
		...
		$is_admin_groups = $this->ion_auth_model->get_admin_superadmin_groups()->result();
			$groups_array = array();
			foreach ($is_admin_groups as $group)
			{
				$groups_array[$group->id] = $group->name;
			}
			
		$admin_group = $groups_array;
...
function menu_preferences ($link)
{
	
			//Select User_ID
			$id = $this->ion_auth->user()->row()->id;
			//Select Grup_ID
			$groups_id = $this->ion_auth->get_users_groups($id);
			foreach( $groups_id->result() as $gi )
			{
				if ( $gi->is_super_admin == 1)
				{
					$group_id = $gi->id;
					break;
				}
			...
			//Select Menu_ID
			return $menu_id = $this->Menus_preferences_model->get_by_link_groupid_active_show($link, $group_id)->num_rows();
}

function check_access_menu($link)
{
			//Select User_ID
			$id = $this->ion_auth->user()->row()->id;
			//Select Grup_ID
			...
			//Select Menu_ID
			return $menu_id = $this->Menus_preferences_model->get_by_link_groupidmulti_active($link, $group_id)->result();
}
}
