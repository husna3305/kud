...
<div class="content-wrapper">
	...
	<section class="content">
            ...
            <table id="data_table" class="table table-bordered table-striped table-hover">
                <thead style="font-weight: bold;font-size: 16px;background-color: #5ca1c7;color: white;">
                    <tr>    
                        ...
                        <th style="text-align: center;">Uraian Data Suplier Yang Dihapus</th>  
                ...
                <?php foreach( $suplier_dihapus->result() as $spr ) { ?>
							<tr>
								<td style="text-align: center;"><?php echo $spr->id_suplier ?></td>
								<td>
								...
								<?php if ($edited=='y' or $deleted=='y'): ?>
									...
									<?php if ($edited=='y'): ?>
										<button type="button" data-id_suplier="<?php echo $spr->id_suplier ?>" data-deleted_status="<?php echo $spr->deleted_status ?>" class="btn btn-success btn-xs set_delete"><i class="fa fa-check"></i>&nbsp;&nbsp;Batal Hapus</button>
									<?php endif ?>
									<?php if ($deleted=='y'): ?>
										&nbsp;&nbsp;
										<button type="button" data-id_suplier="<?php echo $spr->id_suplier ?>" class="btn btn-danger btn-xs delete_permanen"><i class="fa fa-trash"></i>&nbsp;&nbsp;Hapus Permanen</button>
        ...
        <a href="<?php echo site_url('admin/suplier') ?>" class="btn btn-default"><i class="fa fa-chevron-circle-left"></i>&nbsp;&nbsp;Kembali</a>
					</div>
				</div>
			 </div>
		</div>
	...
	$('#data_table').on('click','.delete_permanen',function(){
		swal({
			  ...
			  if (result.value) {
			  	...
			  	var deleted_status = $(this).data('deleted_status'); //get value input
			    $.ajax({
					 url:"<?php echo site_url('admin/suplier/delete_permanen');?>",
					 type:"POST",
					 data:"id_suplier="+id_suplier+"&deleted_status="+deleted_status+"&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo$this->security->get_csrf_hash(); ?>",
					 cache:false,
					 success:function(html){
...
/*
        $("#form_multi_select").on('submit', function(e){
          ...
          //  $("#view-form").text($(form).serialize())
           // $('input[name="id\[\]"]', form).remove()
            //e.preventDefault()
			document.form_multi_select.action = "<?php site_url('admin/suplier/aktif_all') ?>";
