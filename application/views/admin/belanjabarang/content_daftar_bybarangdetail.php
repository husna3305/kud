...
<?php if ($empty=='n'){ ?>
	...
	<?php $no_header=1; foreach ($detail_perpage as $key => $value): ?>
		<div class="page">
			...
			<table width="100%">
				<tr align="center">
					<td><strong class="f11">DAFTAR TRANSAKSI BELANJA BERDASARKAN BARANG</strong></td>
				</tr><tr align="center">
					<td><strong class="f11">PERIODE : <?php echo date_indo($tgl_mulai) ?> - <?php echo date_indo($tgl_selesai) ?></strong></td>
				</tr>
			</table><br>
			
			<table class="detail" style="margin-bottom: 8px;width: 100%">
				<tr>
					<td width="20%">Nama Barang</td><td>:</td><td><span style="width: 90%"><?php echo $barang->nama_barang ?></span></td>
				</tr>
				<tr>
					<td>Kategori Barang</td><td>:</td><td><span style="width: 90%"><?php if (!!$barang->root_kategori): ?>
						<?php echo $barang->root_kategori ?>,
					<?php endif ?><?php echo $barang->nama_kategori ?></span></td>
				</tr>
			</table>
			<?php endif ?>
			<table class="it-grid it-cetak" width="100%">
					<tr >	
							<td style="background-color: #f8f8f8;text-align: center;width: 6%;padding: 6px 5px;"><b>No.</b></td>
							<td style="background-color: #f8f8f8;text-align: center;width: 34%"><b>ID Transaksi Belanja</b></td>
							<td style="background-color: #f8f8f8;text-align: center;font-weight: bold;width: 21%">Tanggal Transaksi</td>
				...
				<?php foreach ($value as $kk => $v_detail){ ?>
					<tr>
						<?php if($v_detail['id_detail']>=1){ ?>
						<td align="center"><?php echo $v_detail['no'] ?>.</td>
						<td>&nbsp;<?php echo $v_detail['id_transaksi_belanja'];?></td>
						<td>&nbsp;<?php echo date_indo($v_detail['tanggal']);?></td>
			...
			<?php if ($key == count($detail_perpage)): ?>
				<br>
				<table width="100%">
					<tr>
						<td>&nbsp;</td>
