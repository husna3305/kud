...
class Ion_auth_model extends CI_Model
{
	...
	{
		$sql = 'select * from '.$this->tables['users'].' where username = "'.$username.'"';
		return $this->db->query($sql);
	}

	public function get_user_public_by_id($id)
	{
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.bgcolor, '.$this->tables['groups'].'.is_super_admin , '.$this->tables['groups'].'.is_admin, '.$this->tables['users_groups'].'.user_id')
	...
	{
		$sql = 'select * from '.$this->tables['groups'].' where is_admin = 1';
		return $this->db->query($sql);
	}
	
	public function get_all_super_admin_groups()
	{
		$sql = 'select * from '.$this->tables['groups'].' where is_super_admin = 1';
		return $this->db->query($sql);
	}

	public function get_all_user_SuperAdmin()
	{
		$group_superadmin = $this->db->query('select * from '.$this->tables['groups'].' where is_super_admin = 1')->result();
		foreach ($group_superadmin as $gr) {
			$gr_sa[] = $gr->id;
		}
		$gr_sa_imp = implode(',', $gr_sa);
		$sql = 'select user_id from '.$this->tables['users_groups'].' where group_id IN ('.$gr_sa_imp.')';
		return $this->db->query($sql);
	}
	
	public function get_admin_superadmin_groups()
	{
		$sql = 'select * from '.$this->tables['groups'].' where is_admin = 1 or is_super_admin=1';
		return $this->db->query($sql);
	}
	
	
	public function get_super_admin_users_groups($id)
	{
		$sql = 'select '.$this->tables['users_groups'].'.* from '.$this->tables['users_groups'].' join '.$this->tables['groups'].' on '.$this->tables['users_groups'].'.group_id = '.$this->tables['groups'].'.id where '.$this->tables['groups'].'.is_super_admin = 1 and '.$this->tables['users_groups'].'.user_id = '.$id;
		/*$sql = "select $this->tables['users_groups'].id from $this->tables['users_groups'] 
				join $this->tables['groups'] on $this->tables['users_groups'].group_id = $this->tables['groups'].id 
				where $this->tables['groups'].is_super_admin = 1 and $this->tables['users_groups'].user_id = $id";
		 */	return $this->db->query($sql);
	}
